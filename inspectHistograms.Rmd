...
```{r}
dOne<- dg %>% filter(targetSP==4, Epoch==1)
cOne<- curves %>% filter(targetSP==4, Epoch==1)
plotBunch(dOne,cOne)

```

Simulate plotBunch to find out what has gone wrong.
...
```{r}
  ...
  g
  #curves <- dplyr::mutate(curves, mixSig = ifelse(pLRtest <= .05, TRUE, FALSE)) #annotate_fit uses this to color the p-value
  cOne<-cOne %>% mutate(practice= ifelse(Epoch=="practice1" | Epoch=="practice2" | Epoch=="practice3", TRUE, FALSE))
  cOne <- cOne %>% filter(practice==FALSE)
  
  table(cOne$Group,cOne$Condition,cOne$targetSP,cOne$Block,cOne$Subject,cOne$Epoch,cOne$efficacy)
  #Combine columns into one column so can see more easily
  table(cOne$Epoch,cOne$efficacy)
  #I want to know which column differs for the different efficacies
  I'M DOING EPOCHS, BUT THE CURVE WAS FIT PER BLOCK

  listOfCols <- as.list( cOne[condtnVariableNames] )
  t<- table(listOfCols)
  if (TRUE) {  
    t<- t[t!=0]   
  }           
  ...
  checkAllGroupsOccurEquallyOften(cOne, condtnVariableNames, FALSE, FALSE)

  g<- annotate_fit(g,cOne) #assumes curvesDf includes efficacy,latency,precision
  show(g)
```


Example: take a particular Group,Condition combination.

This would be the first time I've analysed data separately for different targetSP. Is that the reason for `minSPE must be no greater than -3 based on the values you passed me` ?

Let's try a sample with all targetSPs, see if that avoids the error.
...
#Error in bind_rows_(x, .id) :  Column `warnings` can't be converted from character to list
```  
