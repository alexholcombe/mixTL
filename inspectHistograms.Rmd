...
condtnVariableNames <- c("Group","Condition","targetSP", "Block","Subject","Epoch")  
```

But when I try to add the curves, it seems like something redundant is happening
with multiple curves laid on top of each other. In fact the gaussian freq does not look like a guassian and the annotation is written at least twice.
